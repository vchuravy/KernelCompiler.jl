...
function transform(interp, mi, src)
    ...
    if f === Kernel
        ci = copy(src)
        ci.code = Any[
            Expr(:call, Base.getproperty, Core.SlotNumber(1), :(:f)),
            Expr(:call, Core._apply_iterate, Base.iterate, SSAValue(1), Core.SlotNumber(2)),
            Core.ReturnNode(SSAValue(2))
        ]
        resize!(ci.codelocs, length(ci.code))
        resize!(ci.ssaflags, length(ci.code))
